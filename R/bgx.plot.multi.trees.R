...
#'
#' @param model produced by the \code{bgx.train} function.
...
#'
#' bst <- tsoobgx(data = agaricus.train$data, label = agaricus.train$label, max_depth = 15,
#'                eta = 1, nthread = 2, nrounds = 30, objective = "binary:logistic",
#'                min_child_weight = 50, verbose = 0)
#'
#' p <- bgx.plot.multi.trees(model = bst, features_keep = 3)
#' print(p)
#'
#' \dontrun{
...
#' library(DiagrammeR)
#' gr <- bgx.plot.multi.trees(model=bst, features_keep = 3, render=FALSE)
#' export_graph(gr, 'tree.pdf', width=1500, height=600)
#' }
#'
#' @export
bgx.plot.multi.trees <- function(model, feature_names = NULL, features_keep = 5, plot_width = NULL, plot_height = NULL,
                                 render = TRUE, ...){
  check.deprecation(...)
  tree.matrix <- bgx.model.dt.tree(feature_names = feature_names, model = model)
