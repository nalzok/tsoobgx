...
#' 
#' @export
bgx.plot.tree <- function(feature_names = NULL, model = NULL, trees = NULL, plot_width = NULL, plot_height = NULL,
                          render = TRUE, show_node_id = FALSE, ...){
  check.deprecation(...)
  if (!inherits(model, "bgx.Booster")) {
    stop("model: Has to be an object of class bgx.Booster")
  }

  if (!requireNamespace("DiagrammeR", quietly = TRUE)) {
    stop("DiagrammeR package is required for bgx.plot.tree", call. = FALSE)
  }
  
  dt <- bgx.model.dt.tree(feature_names = feature_names, model = model, trees = trees)
