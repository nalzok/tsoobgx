...
#' \code{bgx.plot.deepness} uses base R graphics, while \code{bgx.ggplot.deepness} uses the ggplot backend.
#' 
#' @param model either an \code{bgx.Booster} model generated by the \code{bgx.train} function
#'        or a data.table result of the \code{bgx.model.dt.tree} function.
...
#' @return
#' 
#' Other than producing plots (when \code{plot=TRUE}), the \code{bgx.plot.deepness} function
...
#' 
#' The \code{bgx.ggplot.deepness} silently returns either a list of two ggplot graphs when \code{which="2x1"}
#' or a single ggplot graph for the other \code{which} options.
#'
#' @seealso 
#' 
#' \code{\link{bgx.train}}, \code{\link{bgx.model.dt.tree}}.
#' 
#' @examples
#' 
...
#' bst <- tsoobgx(data = agaricus.train$data, label = agaricus.train$label, max_depth = 6,
#'                eta = 0.1, nthread = 2, nrounds = 50, objective = "binary:logistic",
#'                subsample = 0.5, min_child_weight = 2)
#'
#' bgx.plot.deepness(bst)
#' bgx.ggplot.deepness(bst)
#' 
#' bgx.plot.deepness(bst, which='max.depth', pch=16, col=rgb(0,0,1,0.3), cex=2)
#' 
#' bgx.plot.deepness(bst, which='med.weight', pch=16, col=rgb(0,0,1,0.3), cex=2)
#'
#' @rdname bgx.plot.deepness
#' @export
bgx.plot.deepness <- function(model = NULL, which = c("2x1", "max.depth", "med.depth", "med.weight"),
                              plot = TRUE, ...) {
  
  if (!(inherits(model, "bgx.Booster") || is.data.table(model)))
    stop("model: Has to be either an bgx.Booster model generaged by the bgx.train function\n",
         "or a data.table result of the bgx.importance function")
  ...
  if (inherits(model, "bgx.Booster"))
    dt_tree <- bgx.model.dt.tree(model = model)
