# ggplot backend for the tsoobgx plotting facilities


#' @rdname bgx.plot.importance
#' @export
bgx.ggplot.importance <- function(importance_matrix = NULL, top_n = NULL, measure = NULL, 
                                  rel_to_first = FALSE, n_clusters = c(1:10), ...) {
  
  importance_matrix <- bgx.plot.importance(importance_matrix, top_n = top_n, measure = measure,
...
#' @rdname bgx.plot.deepness
#' @export
bgx.ggplot.deepness <- function(model = NULL, which = c("2x1", "max.depth", "med.depth", "med.weight")) {
  ...
  dt_depths <- bgx.plot.deepness(model = model, plot = FALSE)
