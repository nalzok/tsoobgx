...
#' @param text \code{character} vector previously generated by the \code{bgx.dump} 
...
#'  
#' @export
bgx.model.dt.tree <- function(feature_names = NULL, model = NULL, text = NULL,
                              trees = NULL, use_int_id = FALSE, ...){
  check.deprecation(...)
  
  if (!inherits(model, "bgx.Booster") && !is.character(text)) {
    stop("Either 'model' must be an object of class bgx.Booster\n",
         "  or 'text' must be a character vector with the result of bgx.dump\n",
  ...
  if (is.null(text)){
    text <- bgx.dump(model = model, with_stats = TRUE)
  }
