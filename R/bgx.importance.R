...
#' @param model object of class \code{bgx.Booster}.
...
#'          for each class separately. IMPORTANT: the tree index in tsoobgx models
...
#' bst <- tsoobgx(data = agaricus.train$data, label = agaricus.train$label, booster = "gblinear", 
...
#' nclass <- 3
#' nrounds <- 10
#' mbst <- tsoobgx(data = as.matrix(iris[, -5]), label = as.numeric(iris$Species) - 1,
#'                max_depth = 3, eta = 0.2, nthread = 2, nrounds = nrounds,
#'                objective = "multi:softprob", num_class = nclass)
#' # all classes clumped together:
#' bgx.importance(model = mbst)
#' # inspect importances separately for each class:
#' bgx.importance(model = mbst, trees = seq(from=0, by=nclass, length.out=nrounds))
#' bgx.importance(model = mbst, trees = seq(from=1, by=nclass, length.out=nrounds))
#' bgx.importance(model = mbst, trees = seq(from=2, by=nclass, length.out=nrounds))
#' 
#' # multiclass classification using gblinear:
#' mbst <- tsoobgx(data = scale(as.matrix(iris[, -5])), label = as.numeric(iris$Species) - 1,
#'                booster = "gblinear", eta = 0.2, nthread = 1, nrounds = 15,
#'                objective = "multi:softprob", num_class = nclass)
#' bgx.importance(model = mbst)
#'
#' @export
bgx.importance <- function(feature_names = NULL, model = NULL, trees = NULL,
                           data = NULL, label = NULL, target = NULL){
  
  if (!(is.null(data) && is.null(label) && is.null(target)))
    warning("bgx.importance: parameters 'data', 'label' and 'target' are deprecated")
  
  if (!inherits(model, "bgx.Booster"))
    stop("model: must be an object of class bgx.Booster")
  ...
  model_text_dump <- bgx.dump(model = model, with_stats = TRUE)
  ...
  # tree model
    result <- bgx.model.dt.tree(feature_names = feature_names,
