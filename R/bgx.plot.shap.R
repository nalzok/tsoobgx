...
#' @param model an \code{bgx.Booster} model. It has to be provided when either \code{shap_contrib}
#'          or \code{features} is missing.
#' @param trees passed to \code{\link{bgx.importance}} when \code{features = NULL}.
...
#' @param approxcontrib passed to \code{\link{predict.bgx.Booster}} when \code{shap_contrib = NULL}.
...
#'
#' bst <- tsoobgx(agaricus.train$data, agaricus.train$label, nrounds = 50,
#'                eta = 0.1, max_depth = 3, subsample = .5,
#'                method = "hist", objective = "binary:logistic", nthread = 2, verbose = 0)
#'
#' bgx.plot.shap(agaricus.test$data, model = bst, features = "odor=none")
#' contr <- predict(bst, agaricus.test$data, predcontrib = TRUE)
#' bgx.plot.shap(agaricus.test$data, contr, model = bst, top_n = 12, n_col = 3)
#'
...
#' mbst <- tsoobgx(data = x, label = as.numeric(iris$Species) - 1, nrounds = nrounds,
#'                 max_depth = 2, eta = 0.3, subsample = .5, nthread = 2,
#'                 objective = "multi:softprob", num_class = nclass, verbose = 0)
#' trees0 <- seq(from=0, by=nclass, length.out=nrounds)
#' col <- rgb(0, 0, 1, 0.5)
#' bgx.plot.shap(x, model = mbst, trees = trees0, target_class = 0, top_n = 4,
#'               n_col = 2, col = col, pch = 16, pch_NA = 17)
#' bgx.plot.shap(x, model = mbst, trees = trees0 + 1, target_class = 1, top_n = 4,
#'               n_col = 2, col = col, pch = 16, pch_NA = 17)
#' bgx.plot.shap(x, model = mbst, trees = trees0 + 2, target_class = 2, top_n = 4,
#'               n_col = 2, col = col, pch = 16, pch_NA = 17)
#'
#' @rdname bgx.plot.shap
#' @export
bgx.plot.shap <- function(data, shap_contrib = NULL, features = NULL, top_n = 1, model = NULL,
  ...
  if (is.null(shap_contrib) && (is.null(model) || !inherits(model, "bgx.Booster")))
    stop("when shap_contrib is not provided, one must provide an bgx.Booster model")

  if (is.null(features) && (is.null(model) || !inherits(model, "bgx.Booster")))
    stop("when features are not provided, one must provide an bgx.Booster model to rank the features")
  ...
  if (is.null(features)) {
    imp <- bgx.importance(model = model, trees = trees)
