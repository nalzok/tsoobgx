...
#' with the objects available inside of the \code{bgx.train} and \code{bgx.cv} internal environments.
#'
#' @seealso
...
cb.reset.parameters <- function(new_params) {
  ...
  callback <- function(env = parent.frame()) {
    ...
    if (!is.null(env$bst)) {
      bgx.parameters(env$bst$handle) <- pars
    } else {
      for (fd in env$bst_folds)
        bgx.parameters(fd$bst) <- pars
    }
  }
...
#' }
#'
#' The Same values are also stored as bgx-attributes:
#' \itemize{
...
cb.early.stop <- function(stopping_rounds, maximize = FALSE,
  ...
  init <- function(env) {
    ...
    # maximize is usually NULL when not set in bgx.train and built-in metrics
    if (is.null(maximize))
    ...
    if (!is.null(env$bst)) {
      if (!inherits(env$bst, 'bgx.Booster'))
        stop("'bst' in the parent frame must be an 'bgx.Booster'")
      if (!is.null(best_score <- bgx.attr(env$bst$handle, 'best_score'))) {
        best_score <<- as.numeric(best_score)
        best_iteration <<- as.numeric(bgx.attr(env$bst$handle, 'best_iteration')) + 1
        best_msg <<- as.numeric(bgx.attr(env$bst$handle, 'best_msg'))
      } else {
        bgx.attributes(env$bst$handle) <- list(best_iteration = best_iteration - 1,
  ...
  finalizer <- function(env) {
    if (!is.null(env$bst)) {
      attr_best_score = as.numeric(bgx.attr(env$bst$handle, 'best_score'))
      if (best_score != attr_best_score)
        stop("Inconsistent 'best_score' values between the closure state: ", best_score,
             " and the bgx.attr: ", attr_best_score)
...
#'        E.g., with \code{save_name} = 'tsoobgx_%04d.model',
#'        the file saved at iteration 50 would be named "tsoobgx_0050.model".
#'
#' @details
#' This callback function allows to save an bgx-model file, either periodically after each \code{save_period}'s or at the end.
#'
#' Callback function expects the following values to be set in its calling frame:
#' \code{bst},
#' \code{iteration},
...
#'
#' @export
cb.save.model <- function(save_period = 0, save_name = "tsoobgx.model") {
  ...
  callback <- function(env = parent.frame()) {
    ...
    if ((save_period > 0 && (env$iteration - env$begin_iteration) %% save_period == 0) ||
      ...
      bgx.save(env$bst, sprintf(save_name, env$iteration))
  }
  attr(callback, 'call') <- match.call()
...
#' provided in \code{bgx.cv}, the predictions would only be returned properly when this list is a
  ...
  finalizer <- function(env) {
    ...
    if (save_models) {
      env$basket$models <- lapply(env$bst_folds, function(fd) {
        bgx.attr(fd$bst, 'niter') <- env$end_iteration - 1
        bgx.Booster.complete(bgx.handleToBooster(fd$bst), saveraw = TRUE)
      })
    }
  }
...
#' The \code{\link{bgx.gblinear.history}} convenience function provides an easy way to access it.
#' With \code{bgx.train}, it is either a dense of a sparse matrix.
#' While with \code{bgx.cv}, it is a list (an element per each fold) of such matrices.
#'
#' @seealso
#' \code{\link{callbacks}}, \code{\link{bgx.gblinear.history}}.
#'
#' @examples
...
#' colnames(x)
#' dtrain <- bgx.DMatrix(scale(x), label = 1*(iris$Species == "versicolor"))
...
#' bst <- bgx.train(param, dtrain, list(tr=dtrain), nrounds = 200, eta = 1.,
#'                  callbacks = list(cb.gblinear.history()))
#' # Extract the coefficients' path and plot them vs boosting iteration number:
#' coef_path <- bgx.gblinear.history(bst)
#' matplot(coef_path, type = 'l')
#'
#' # With the deterministic coordinate descent updater, it is safer to use higher learning rates.
#' # Will try the classical componentwise boosting which selects a single best feature per round:
#' bst <- bgx.train(param, dtrain, list(tr=dtrain), nrounds = 200, eta = 0.8,
#'                  updater = 'coord_descent', feature_selector = 'thrifty', top_k = 1,
#'                  callbacks = list(cb.gblinear.history()))
#' bgx.gblinear.history(bst) %>% matplot(type = 'l')
#' #  Componentwise boosting is known to have similar effect to Lasso regularization.
#' # Try experimenting with various values of top_k, eta, nrounds,
#' # as well as different feature_selectors.
#'
#' # For bgx.cv:
#' bst <- bgx.cv(param, dtrain, nfold = 5, nrounds = 100, eta = 0.8,
#'              callbacks = list(cb.gblinear.history()))
#' # coefficients in the CV fold #3
#' bgx.gblinear.history(bst)[[3]] %>% matplot(type = 'l')
#'
#'
#' #### Multiclass classification:
#' #
#' dtrain <- bgx.DMatrix(scale(x), label = as.numeric(iris$Species) - 1)
#' param <- list(booster = "gblinear", objective = "multi:softprob", num_class = 3,
#'               lambda = 0.0003, alpha = 0.0003, nthread = 2)
#' # For the default linear updater 'shotgun' it sometimes is helpful
#' # to use smaller eta to reduce instability
#' bst <- bgx.train(param, dtrain, list(tr=dtrain), nrounds = 70, eta = 0.5,
#'                  callbacks = list(cb.gblinear.history()))
#' # Will plot the coefficient paths separately for each class:
#' bgx.gblinear.history(bst, class_index = 0) %>% matplot(type = 'l')
#' bgx.gblinear.history(bst, class_index = 1) %>% matplot(type = 'l')
#' bgx.gblinear.history(bst, class_index = 2) %>% matplot(type = 'l')
#'
#' # CV:
#' bst <- bgx.cv(param, dtrain, nfold = 5, nrounds = 70, eta = 0.5,
#'               callbacks = list(cb.gblinear.history(FALSE)))
#' # 1st forld of 1st class
#' bgx.gblinear.history(bst, class_index = 0)[[1]] %>% matplot(type = 'l')
#'
#' @export
cb.gblinear.history <- function(sparse=FALSE) {
  ...
  init <- function(env) {
    if (!is.null(env$bst)) { # bgx.train:
      coef_path <- list()
    } else if (!is.null(env$bst_folds)) { # bgx.cv:
  ...
  extract.coef <- function(env) {
    if (!is.null(env$bst)) { # # bgx.train:
      cf <- as.numeric(grep('(booster|bias|weigh)', bgx.dump(env$bst), invert = TRUE, value = TRUE))
      if (sparse) cf <- as(cf, "sparseVector")
    } else { # bgx.cv:
      cf <- vector("list", length(env$bst_folds))
      for (i in seq_along(env$bst_folds)) {
        dmp <- bgx.dump(bgx.handleToBooster(env$bst_folds[[i]]$bst))
...
#' callback.
#'
#' @param model either an \code{bgx.Booster} or a result of \code{bgx.cv()}, trained
...
#'
#' @return
#' For an \code{bgx.train} result, a matrix (either dense or sparse) with the columns
#' corresponding to iteration's coefficients (in the order as \code{bgx.dump()} would
#' return) and the rows corresponding to boosting iterations.
#'
#' For an \code{bgx.cv} result, a list of such matrices is returned with the elements
#' corresponding to CV folds.
#'
#' @export
bgx.gblinear.history <- function(model, class_index = NULL) {

  if (!(inherits(model, "bgx.Booster") ||
        inherits(model, "bgx.cv.synchronous")))
    stop("model must be an object of either bgx.Booster or bgx.cv.synchronous class")
  is_cv <- inherits(model, "bgx.cv.synchronous")
  ...
  if (!is_cv) {
    ...
    if (is.null(num_feat))
      stop("This bgx.cv result does not have nfeatures info")
  }
