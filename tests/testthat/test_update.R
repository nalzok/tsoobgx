...
# See https://github.com/dmlc/tsoobgx/issues/3720
...
test_that("updating the model works", {
  ...
  set.seed(11)
  bst1 <- bgx.train(p1, dtrain, nrounds = 10, watchlist, verbose = 0)
  tr1 <- bgx.model.dt.tree(model = bst1)
  
  # with subsampling
  p2 <- modifyList(p1, list(subsample = 0.1))
  set.seed(11)
  bst2 <- bgx.train(p2, dtrain, nrounds = 10, watchlist, verbose = 0)
  tr2 <- bgx.model.dt.tree(model = bst2)
  ...
  set.seed(11)
  bst1r <- bgx.train(p1r, dtrain, nrounds = 10, watchlist, verbose = 0)
  tr1r <- bgx.model.dt.tree(model = bst1r)
  ...
  set.seed(11)
  bst2r <- bgx.train(p2r, dtrain, nrounds = 10, watchlist, verbose = 0)
  tr2r <- bgx.model.dt.tree(model = bst2r)
  ...
  bst1u <- bgx.train(p1u, dtrain, nrounds = 10, watchlist, verbose = 0, bgx_model = bst1)
  tr1u <- bgx.model.dt.tree(model = bst1u)
  # all should be the same when no subsampling
  ...
  bst2u <- bgx.train(p2u, dtrain, nrounds = 10, watchlist, verbose = 0, bgx_model = bst2)
  tr2u <- bgx.model.dt.tree(model = bst2u)
  # should be the same evaluation but different gains and larger cover
  ...
  bst1ut <- bgx.train(p1ut, dtest, nrounds = 10, watchlist, verbose = 0, bgx_model = bst1)
  tr1ut <- bgx.model.dt.tree(model = bst1ut)
...
test_that("updating works for multiclass & multitree", {
  dtr <- bgx.DMatrix(as.matrix(iris[, -5]), label = as.numeric(iris$Species) - 1)
  ...
  p0 <- list(max_depth = 2, eta = 0.5, nthread = 2, subsample = 0.6,
             ...
             base_score = 0)
  set.seed(121)
  bst0 <- bgx.train(p0, dtr, 5, watchlist, verbose = 0)
  tr0 <- bgx.model.dt.tree(model = bst0)
  ...
  bst0u <- bgx.train(p0u, dtr, nrounds = bst0$niter, watchlist, bgx_model = bst0, verbose = 0)
  tr0u <- bgx.model.dt.tree(model = bst0u)
