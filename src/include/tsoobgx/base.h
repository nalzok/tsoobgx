 ...
 * \file base.h
 * \brief defines configuration macros of tsoobgx.
 */
#ifndef TSOOBGX_BASE_H_
#define TSOOBGX_BASE_H_
 ...
 */
#ifndef TSOOBGX_STRICT_R_MODE
#define TSOOBGX_STRICT_R_MODE 0
#endif  // TSOOBGX_STRICT_R_MODE
 ...
 */
#ifndef TSOOBGX_LOG_WITH_TIME
#define TSOOBGX_LOG_WITH_TIME 1
#endif  // TSOOBGX_LOG_WITH_TIME

/*!
 * \brief Whether customize the logger outputs.
 */
#ifndef TSOOBGX_CUSTOMIZE_LOGGER
#define TSOOBGX_CUSTOMIZE_LOGGER TSOOBGX_STRICT_R_MODE
#endif  // TSOOBGX_CUSTOMIZE_LOGGER

/*!
 * \brief Whether to customize global PRNG.
 */
#ifndef TSOOBGX_CUSTOMIZE_GLOBAL_PRNG
#define TSOOBGX_CUSTOMIZE_GLOBAL_PRNG TSOOBGX_STRICT_R_MODE
...
#define TSOOBGX_ALIGNAS(X) alignas(X)
#else
#define TSOOBGX_ALIGNAS(X)
#endif  // defined(__GNUC__) && ((__GNUC__ == 4 && __GNUC_MINOR__ >= 8) || __GNUC__ > 4)

#if defined(__GNUC__) && ((__GNUC__ == 4 && __GNUC_MINOR__ >= 8) || __GNUC__ > 4) && \
    !defined(__CUDACC__)
#include <parallel/algorithm>
#define TSOOBGX_PARALLEL_SORT(X, Y, Z) __gnu_parallel::sort((X), (Y), (Z))
#define TSOOBGX_PARALLEL_STABLE_SORT(X, Y, Z) \
  __gnu_parallel::stable_sort((X), (Y), (Z))
#elif defined(_MSC_VER) && (!__INTEL_COMPILER)
#include <ppl.h>
#define TSOOBGX_PARALLEL_SORT(X, Y, Z) concurrency::parallel_sort((X), (Y), (Z))
#define TSOOBGX_PARALLEL_STABLE_SORT(X, Y, Z) std::stable_sort((X), (Y), (Z))
#else
#define TSOOBGX_PARALLEL_SORT(X, Y, Z) std::sort((X), (Y), (Z))
#define TSOOBGX_PARALLEL_STABLE_SORT(X, Y, Z) std::stable_sort((X), (Y), (Z))
#endif  // GLIBC VERSION
...
#define TSOOBGX_DEVICE __host__ __device__
#else
#define TSOOBGX_DEVICE
#endif  // defined (__CUDA__) || defined(__NVCC__)

/*! \brief namespace of tsoobgx*/
namespace tsoobgx {
/*!
...
class GradientPairInternal {
  ...
  TSOOBGX_DEVICE void SetGrad(float g) { grad_ = g; }
  TSOOBGX_DEVICE void SetHess(float h) { hess_ = h; }

 public:
  using ValueT = T;

  TSOOBGX_DEVICE GradientPairInternal() : grad_(0), hess_(0) {}

  TSOOBGX_DEVICE GradientPairInternal(float grad, float hess) {
    SetGrad(grad);
    SetHess(hess);
  }

  // Copy constructor if of same value type
  TSOOBGX_DEVICE GradientPairInternal(const GradientPairInternal<T> &g)
      : grad_(g.grad_), hess_(g.hess_) {}  // NOLINT

  // Copy constructor if different value type - use getters and setters to
  // perform conversion
  template <typename T2>
  TSOOBGX_DEVICE explicit GradientPairInternal(const GradientPairInternal<T2> &g) {
    SetGrad(g.GetGrad());
    SetHess(g.GetHess());
  }

  TSOOBGX_DEVICE float GetGrad() const { return grad_; }
  TSOOBGX_DEVICE float GetHess() const { return hess_; }

  TSOOBGX_DEVICE GradientPairInternal<T> &operator+=(
  ...
  TSOOBGX_DEVICE explicit GradientPairInternal(int value) {
...
template<>
inline TSOOBGX_DEVICE float GradientPairInternal<int64_t>::GetGrad() const {
  return grad_ * 1e-4f;
}
template<>
inline TSOOBGX_DEVICE float GradientPairInternal<int64_t>::GetHess() const {
  return hess_ * 1e-4f;
}
template<>
inline TSOOBGX_DEVICE void GradientPairInternal<int64_t>::SetGrad(float g) {
  grad_ = static_cast<int64_t>(std::round(g * 1e4));
}
template<>
inline TSOOBGX_DEVICE void GradientPairInternal<int64_t>::SetHess(float h) {
...
/* Always keep this #include at the bottom of tsoobgx/base.h */
#include <tsoobgx/build_config.h>

#endif  // TSOOBGX_BASE_H_
